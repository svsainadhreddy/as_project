...
public interface ApiService {
    ...
    Call<PatientResponse> createPatient(
            @Header("Authorization") String token,
            @Part("patient_id") RequestBody patientId,   // NEW: send pre-generated patient_id (optional)
            @Part("name") RequestBody name,
            @Part("age") RequestBody age,
            @Part("phone") RequestBody phone,
    ...
    // NEW endpoint to fetch next patient id
    @GET("patients/next-id/")
    Call<Map<String, String>> getNextPatientId(@Header("Authorization") String token);
...
// POST create survey
    ...
    @GET("api/surveys/{patient_id}/section-answers/")
    Call<SurveyAnswersResponse> getSectionAnswers(
            @Header("Authorization") String token,
            @Path("patient_id") int patientId,
